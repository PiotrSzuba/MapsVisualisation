...
public class ScrapBcuwCommand : ICommand
{
    internal class ScrapBcuwCommandHandler : ICommandHandler<ScrapBcuwCommand>
    {
        private readonly MapsVisualisationContext _context;

        public ScrapBcuwCommandHandler(MapsVisualisationContext context)
        {
            _context = context;
        }

        public async Task<Unit> Handle(ScrapBcuwCommand request, CancellationToken cancellationToken)
        {
            ...
            {
                var sources = BcuwScrapper.Run(region);
